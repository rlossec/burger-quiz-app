# Wireframes — Quiz (Epic 2)

Maquettes liées à la création et à la gestion des Burger Quiz et des manches.

→ [../page_reference.md](../page_reference.md) · [README.md](README.md)

---

## Fichiers de maquettes

| Fichier | Contenu |
|---------|---------|
| [questions.md](questions.md) | Questions (list, détail, create/edit) |
| [nuggets.md](nuggets.md) | Manches Nuggets |
| [salt-or-pepper.md](salt-or-pepper.md) | Manches Sel ou Poivre |
| [menus-menutheme.md](menus-menutheme.md) | Menus + MenuTheme |
| [addition.md](addition.md) | Manches Addition |
| [deadly-burger.md](deadly-burger.md) | Burger de la mort |
| [burger-quiz.md](burger-quiz.md) | Burger Quiz (list, détail, create/edit) |

Composants communs (InlineForm, etc.) : [../components.md](../components.md).
